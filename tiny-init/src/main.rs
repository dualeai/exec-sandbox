...
fn main() {
    ...
    // Mount root filesystem: mount -t ext4 -o ro,noatime,nosuid,nodev /dev/vda /mnt
    // Matches AWS Lambda rootfs flags (ro,nosuid,nodev,noatime).
    // The rootfs never enters rw state. MS_NOSUID is safe — no SUID binaries
    // in the sandbox. MS_NODEV is safe — devtmpfs is a separate mount.
    if mount(
        "/dev/vda",
        "/mnt",
        "ext4",
